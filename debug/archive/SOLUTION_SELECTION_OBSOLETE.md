# SOLUTION SELECTION - BLOCKING DECISION REQUIRED ⏸️

**Generated**: 2026-01-11  
**Updated**: 2026-01-12 03:36 UTC  
**Status**: 🔴 **PRODUCTION FAILURE - PREVIOUS FIXES INCOMPLETE**  
**Current Gate**: P0 Schema Mismatch (today.rs + user_settings_repos.rs)  
**Purpose**: Document all decisions made and PENDING decisions  
**Production Status**: 4 endpoints returning 500 errors in production as of 03:36 UTC
...
## 🔴 CURRENT BLOCKING DECISION: P0 Schema Mismatch

**Evidence**: Production logs 2026-01-12 03:36 UTC
```
- /api/today/personalization: column "theme" does not exist
- /api/today/sync: column "key" does not exist
- /api/today/*: INT4 vs INT8 type mismatch
- Connection pool: column "streak_days" does not exist
```

**Root Cause**: 
- `today.rs::fetch_personalization()` uses non-existent schema columns
- Dead `user_settings_repos.rs` file with broken queries referenced by today.rs

**Decision Point**: How should we fix this?

### Option A: Complete Rewrite ⭐ RECOMMENDED
```
Files to Change:
  - app/backend/crates/api/src/routes/today.rs (rewrite fetch_personalization)
  - DELETE: app/backend/crates/api/src/db/user_settings_repos.rs

Approach:
  - Map old "interests", "module_weights", etc. to actual schema columns:
    * theme → theme column
    * timezone → timezone column  
    * etc.
  - Use platform_repos::UserSettingsRepo (already correct)
  - Single source of truth

Effort: 30-45 minutes
Validation: cargo check + npm lint
Risk: Low (proper schema alignment)
```

### Option B: Graceful Degradation
```
Files to Change:
  - app/backend/crates/api/src/routes/today.rs (add error handling)
  - Keep user_settings_repos.rs for now

Approach:
  - Wrap queries in try/catch
  - Return sensible defaults when columns missing
  - Continue ignoring schema mismatches

Effort: 15 minutes  
Validation: cargo check + npm lint
Risk: High (perpetuates technical debt, will break again)
```
