 ...
 *
 * NOTE: This component calls the backend API. Once the Rust backend is deployed,
 * update API_BASE to point to api.ecent.online. For now, it uses relative paths
 * which will work when proxied through the main app or the backend.
 */
...
export function AdminClient({ userEmail }: AdminClientProps) {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${API_BASE}/api/admin/${activeTab}`);
      if (response.ok) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/admin/users?userId=${deleteModal.userId}`, {
        method: "DELETE",
      });
      if (response.ok) {
  ...
  const handleCreateQuest = async () => {
    try {
      const response = await fetch(`${API_BASE}/api/admin/quests`, {
        method: "POST",
  ...
  const handleDeleteSkill = async (skillId: string) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/admin/skills?id=${skillId}`, {
        method: "DELETE",
      });
      if (response.ok) {
...
function ContentTab() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      setIsLoading(true);
      try {
        const res = await fetch(`${API_BASE}/api/admin/content?type=${contentType}`);
