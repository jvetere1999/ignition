...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM habit_completions WHERE habit_id = p_habit_id AND completed_date = v_today) THEN
...
FROM habits h
LEFT JOIN habit_completions hl ON h.id = hl.habit_id AND hl.completed_date = CURRENT_DATE
